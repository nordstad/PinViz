...
def build() -> str:
    ...
    # --- PCB ---
    s.append(
        f'<rect x="1" y="1" width="{W - 2}" height="{H - 2}" rx="7" ry="7" '
        f'fill="#1b1b20" stroke="#3a3a42" stroke-width="1.2"/>'
    )
    s.append(
        f'<rect x="5" y="5" width="{W - 10}" height="{H - 10}" rx="5" ry="5" '
    ...
    for hx, hy in [(9, hole_top), (W - 9, hole_top), (9, hole_bot), (W - 9, hole_bot)]:
        s.append(
            f'<circle cx="{hx}" cy="{hy}" r="3.6" fill="#0d0d10" stroke="#9a9aa0" stroke-width="1"/>'
        )
    ...
    ux = 60.0
    s.append(
        f'<rect x="{ux - 13}" y="-6.5" width="26" height="17.5" rx="2.6" '
        f'fill="#d3d6dc" stroke="#7d818a" stroke-width="0.9"/>'
    )
    s.append(
        f'<rect x="{ux - 11.5}" y="-4.9" width="23" height="14.3" rx="1.6" '
    ...
    for bx, lbl in [(18, "BOOT"), (90, "RST")]:
        s.append(
            f'<rect x="{bx}" y="17" width="12" height="9" rx="1.2" fill="#2b2b33" stroke="#565660" stroke-width="0.5"/>'
        )
        s.append(f'<rect x="{bx + 3.5}" y="19" width="5" height="5" rx="0.8" fill="#9a9aa4"/>')
        s.append(
            f'<text x="{bx + 6}" y="31.5" font-family="Arial, sans-serif" font-size="3" '
            f'fill="#8f8f98" text-anchor="middle">{lbl}</text>'
        )
    # --- RGB LED (offset from the USB so it is not mistaken for it) ---
    s.append(
        '<rect x="43" y="18.5" width="5" height="5" rx="0.6" fill="#f2f2f5" stroke="#8a8a90" stroke-width="0.4"/>'
    )
    s.append('<rect x="44.2" y="19.7" width="2.6" height="2.6" rx="0.4" fill="#9fd0ff"/>')
    s.append(
        '<text x="45.5" y="28.5" font-family="Arial, sans-serif" font-size="2.6" '
        'fill="#7f7f88" text-anchor="middle">RGB</text>'
    )
    # --- ESP32-S3-WROOM module (shield can) ---
    mx, my, mw, mh = 34.0, 52.0, 52.0, 104.0
    s.append(
        f'<rect x="{mx}" y="{my}" width="{mw}" height="{mh}" rx="2.5" '
        f'fill="#bfc3c9" stroke="#7a7e86" stroke-width="1"/>'
    )
    s.append(
        f'<rect x="{mx + 1.6}" y="{my + 1.6}" width="{mw - 3.2}" height="{mh - 3.2}" rx="1.8" '
        f'fill="none" stroke="#9aa0a8" stroke-width="0.5"/>'
    )
    s.append(
        f'<rect x="{mx + 6}" y="{my + 4}" width="{mw - 12}" height="15" rx="1" fill="#d7dbe0" stroke="#a7adb5" stroke-width="0.5"/>'
    )
    for i in range(6):
        ax = mx + 9 + i * 6
        s.append(
            f'<path d="M{ax},{my + 16} l3,-9 l3,9" fill="none" stroke="#9aa0a8" stroke-width="0.6"/>'
        )
    s.append(
        f'<text x="{mx + mw / 2}" y="{my + 42}" font-family="Arial, sans-serif" font-size="6.4" '
        f'font-weight="bold" fill="#3c3f45" text-anchor="middle">ESP32-S3</text>'
    )
    s.append(
        f'<text x="{mx + mw / 2}" y="{my + 52}" font-family="Arial, sans-serif" font-size="4.4" '
        f'fill="#4c4f55" text-anchor="middle">WROOM-1</text>'
    )
    s.append(
        f'<text x="{mx + mw / 2}" y="{my + 90}" font-family="Arial, sans-serif" font-size="3.2" '
    ...
    for cx in (LEFT_X, RIGHT_X):
        s.append(
            f'<rect x="{cx - 5}" y="{strip_y}" width="10" height="{strip_h}" rx="1.5" '
    ...
    for i in range(N):
        y = START_Y + i * ROW
        s.append(
            f'<circle cx="{LEFT_X}" cy="{y}" r="4.4" fill="#d8b24a" stroke="#8a6f22" stroke-width="0.5"/>'
        )
        s.append(
            f'<circle cx="{RIGHT_X}" cy="{y}" r="4.4" fill="#d8b24a" stroke="#8a6f22" stroke-width="0.5"/>'
        )
...
def main() -> None:
    out = (
        ...
        / "pinviz"
        / "assets"
        / "esp32_s3_devkitc1_mod.svg"
    )
    out.write_text(build())
    print(f"wrote {out}")
    print(
        f"config -> left_col_x={LEFT_X} right_col_x={RIGHT_X} "
