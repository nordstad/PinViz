...
- **ESP32-S3-DevKitC-1 board** - full 2×22 (44-pin) J1/J3 header, in two variants:
  - `esp32_s3_devkitc1` (default): realistic board artwork (black PCB, USB-C, ESP32-S3-WROOM module, BOOT/RST buttons, per-pin silkscreen GPIO labels) so the diagram maps onto the physical board at a glance. Aliases: `esp32s3`, `esp32_s3`, `esp32_s3_devkit` (https://github.com/nordstad/PinViz/pull/282)
  ...
  - Board artwork is generated by `scripts/gen_esp32_s3_devkitc1_svg.py` (reproducible; pad coordinates match the config)
...
- **SN65HVD230 CAN transceiver** device (`io` category) with logic-side (3V3/GND/CTX/CRX) and bus-side (CANH/CANL) pins
  - 📝 New example: `examples/esp32_s3_can_transceiver.yaml` (ESP32-S3 TWAI → SN65HVD230) (https://github.com/nordstad/PinViz/pull/284)

### Changed
- chore(deps): bump typer from 0.25.1 to 0.26.8, mcp from 1.27.1 to 1.28.1, starlette from 0.50.0 to 1.3.1, cryptography from 46.0.7 to 48.0.1, structlog from 25.5.0 to 26.1.0, pydantic-settings, beautifulsoup4, python-multipart, pyjwt, and dev tooling (ruff, pytest, hypothesis, zensical, mkdocstrings-python, platformdirs, pytest-asyncio)
